...
async function parseSessionFile(filePath) {
  ...
  for (const rec of merged) {
    if (rec.type === 'user') {
      ...
      const text = isSystemInjected(rec) ? '' : extractUserText(content);
      if (text) {
        steps.push({
          role: 'user',
          kind: 'user',
...
/**
 * ハーネスが会話に差し込んだ user レコードかどうか。
 * (バックグラウンドタスクの通知や "Continue from where you left off." など。
 *  人間が入力したものではないので再生からも集計からも外す)
 *
 * jsonl 上では isMeta が目印になる。origin.kind === 'human' は必ず残し、
 * origin を持たない古い形式のログは従来どおり残す。
 */
function isSystemInjected(rec) {
 ...
 *  - 注意書き・システムリマインダ・タスク通知 -> 会話ではないので落とす
 */
...
async function readSessionMeta(filePath) {
  ...
  for (const line of rawLines) {
    ...
    if (obj.type === 'user') {
      ...
      const text = isRealPrompt && !isSystemInjected(obj) ? extractUserText(content) : '';
      if (text) {
        userTurns += 1;
